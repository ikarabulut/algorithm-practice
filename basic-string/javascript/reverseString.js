...
// # 1. Basics would be to create a function called reverseString that takes in a single string as an argument. From there you will need to create a new variable called reversedString which will be set to an empty string. This is where we will create the reversed string
// # 2. We will need an index since we will be going through each letter of the string. Since we want the string to be reversed we want to add the last letter to the new reversedString variable. So we will set index to the length of the string: string.length but we will need to subtract one from that since .length starts at 1 but we want to end in position 0 of the string. The first position.
...
// # 6 end it by returning reversedString
